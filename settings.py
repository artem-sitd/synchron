        ...
        while True:
            ...
            try:
                response = requests.get(
                    upload_get_delete_urls(cloud_folder), headers=headers
                )
                if response.status_code == 200:
                    with open(".env", "a") as data:
                        print(f'\nCLOUD_FOLDER="{cloud_folder}"', file=data)
                    print(
                        "Папка зарегистрирована. Все настройик выполнены - начинаем работу.\n"
                    )
...
def set_period(user1):
    ...
    while True:
        try:
            ...
            print(
                "Необходимо вводить только цифры. Введите период "
...
def set_self_folder(user1):
    ...
    while True:
        ...
        if os.path.exists(path_to_local_folder):
            ...
            print(
                "По указанному пути указанной папки не обнаружено, укажите путь "
                "к локальной папке: "
            )
    set_period(user1)
...
def set_token(user1):
    ...
    if check_token(oauth_token):
        ...
        print(
            ">>> Предоставленный токен или clientID не верен, начнем еще "
            "раз заново\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
        )
