...
import config
from hashwork import calculate_file_hash, create_hash, hash_compare
from URLS import (
    check_available_url,
    check_token_url,
    get_headers,
    upload_get_delete_urls,
)
...
class YandexCloud:
    ...
    def __str__(self):
        return (
            f"clientid= {self.clientid}, token= {self.oauth_token}, "
    ...
    def list_local_files(self):
        ...
        for file in os.listdir(config.SELF_FOLDER):
            if os.path.isfile(os.path.join(config.SELF_FOLDER, file)):
                dict_hash_local[file] = calculate_file_hash(
                    os.path.join(config.SELF_FOLDER, file)
                )
        if dict_hash_local:
    ...
    def list_yandex_disk_files(self):
        ...
        if check_available_url(check_token_url, headers=headers):
            response = requests.get(
                upload_get_delete_urls(config.CLOUD_FOLDER), headers=headers
            )
            if response.json()["name"] == config.CLOUD_FOLDER:
                ...
                print(
                    f"list_yandex_disk_files:, code: {response.status_code},response: {response.text}"
                )
                return None
            else:
    ...
    def compare_lists(self, local_files, cloud_files):
        ...
        added_by_name = set(local_files.keys()) - set(
            cloud_files
        )  # Имена файлов, которых нет в облаке. Предварительный список
    ...
    def load(self, files_to_upload):
        ...
        if check_available_url(check_token_url, headers=headers):
            for file in files_to_upload:
                ...
                url = upload_get_delete_urls(
                    f"{config.CLOUD_FOLDER}/{file}",
                    upload="/upload",
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"Failed to get upload URL. Status code: {response.status_code}, Response: {response.text}"
                    )
        else:
            print("сервис недоступен Проверьте соединение с интернетом")
            exit(1)
        ...
        if check_available_url(check_token_url, headers=headers):
            for file in deleted_files:
                ...
                if response.status_code != 204:
                    print(
                        f"Failed to delete {file}. Status code: {response.status_code}, Response: {response.text}"
                    )
                else:
    ...
    def update(self):
        ...
        )  # Словарь хэшей файлов в локальной папке
        ...
        create_hash()
        print(
            "Синхронизация произведена, программа в режиме ожидания на ",
            config.PERIOD,
            "секунд",
        )
        return
